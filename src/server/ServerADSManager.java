...
/**
 * THIS CLASS IS NOT THREADSAFE ONLY concurrent getUpdate() are SAFE.
 * 
 * The server ads is a key, value store that maps "adsKeys" (identifies an ads)
 * to "adsValues" (commitments).
 * 
 * This class is responsible for managing the server authentication ads
 * 
 * Currently this implementation keeps the entire server ads in memory for
 * speed.
 * 
...
public class ServerADSManager {
	...
	public ServerADSManager(String base) {
		this.base = base;
		File f = new File(base + "starting-ads");
		try {
			FileInputStream fis = new FileInputStream(f);
			byte[] encodedAds = new byte[(int) f.length()];
			fis.read(encodedAds);
			fis.close();
			this.ads = MPTDictionaryFull.deserialize(encodedAds);
